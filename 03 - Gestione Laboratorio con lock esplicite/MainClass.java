...
/*
Programma per la gestione di un laboratorio di informatica. Implementazione con lock esplicite.

Gli utenti sono stati implementati tramite thread, mentre il gestore del laboratorio è stato implementato tramite un oggetto
(istanza della classe LabTutor) che viene condiviso tra tutti i thread utenti.

All'interno della classe tutor si trovano le lock che rappresentano i PC del laboratorio, con annesse varie variabili di condizione,
e una lock aggiuntiva che garantisce la mutua esclusione nell'accesso ad alcune strutture di book keeping.
Non c'è una coda esplicita degli utenti in attesa, che sono memorizzati nella coda implicita delle variabili di condizione.

Per evitare lo starving dei professori, è stato inserita una variabile condivisa "ProfInAttesa" e la procedura ChiediUsoPC
...
public class MainClass {
	...
	public static void main (String[] args)
	{
		...
		//creo l'oggetto tutor del laboratorio
		LabTutor Tutor = new LabTutor(NumPC);
		...
		{
			LabUser studente = new LabUser(NumPC, LabUser.UserType.Studente, i, Tutor, MaxNumAccessi, MaxAttesa, MaxUtilizzo, ThreadStatus, i);
		...
		{
			ThreadStatus[i+NumStudenti] = new Character('_');
			LabUser tesista = new LabUser(NumPC, LabUser.UserType.Tesista, i, Tutor, MaxNumAccessi, MaxAttesa, MaxUtilizzo, ThreadStatus, i+NumStudenti);
		...
		{
			ThreadStatus[i+NumStudenti+NumTesisti] = new Character('_');
			LabUser professore = new LabUser(NumPC, LabUser.UserType.Professore, i, Tutor, MaxNumAccessi, MaxAttesa, MaxUtilizzo, ThreadStatus, i+NumStudenti+NumTesisti);
